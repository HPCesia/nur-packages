...
#!nix-shell -i bash -p curl -p jq -p nix -p flutter347 -p python3 -p python3Packages.pyyaml
# shellcheck shell=bash
set -euo pipefail

REPO_URL="https://github.com/Chevey339/kelivo"
...
(
	cd "$TMPDIR"
	# upstream does not commit pubspec.lock, so resolve it here
